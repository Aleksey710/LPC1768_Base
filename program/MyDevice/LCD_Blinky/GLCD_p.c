...
/* EaOA?UD?·?E?E±¶?NlµRO· */
/* aNOU?»I¬µABANK,EaOA?»I¬µRO·D?E±Ce?ODD»»EaµRO· */
...
static void LCD_FSMCConfig(void)
{
  ...
  /* FSMC¶AEU¶CEcOA */
  FSMC_NORSRAMTimingInitStructure.FSMC_AddressSetupTime = 10;  /* µRO·??A?E±La  */
  FSMC_NORSRAMTimingInitStructure.FSMC_AddressHoldTime = 0;
  FSMC_NORSRAMTimingInitStructure.FSMC_DataSetupTime = 10;	   /* EylY??A?E±La  */
  FSMC_NORSRAMTimingInitStructure.FSMC_BusTurnAroundDuration = 0x00;
  FSMC_NORSRAMTimingInitStructure.FSMC_CLKDivision = 0x00;
  FSMC_NORSRAMTimingInitStructure.FSMC_DataLatency = 0x00;
  FSMC_NORSRAMTimingInitStructure.FSMC_AccessMode = FSMC_AccessMode_A;	/* FSMC ·AIEALE? */
  ...
  /* FSMCD?EU¶CEcOA */
  FSMC_NORSRAMTimingInitStructure.FSMC_AddressSetupTime = 3;   /* µRO·??A?E±La  */
  FSMC_NORSRAMTimingInitStructure.FSMC_AddressHoldTime = 0;
  FSMC_NORSRAMTimingInitStructure.FSMC_DataSetupTime = 3;	   /* EylY??A?E±La  */
  FSMC_NORSRAMTimingInitStructure.FSMC_BusTurnAroundDuration = 0x00;
  FSMC_NORSRAMTimingInitStructure.FSMC_CLKDivision = 0x00;
  FSMC_NORSRAMTimingInitStructure.FSMC_DataLatency = 0x00;
  FSMC_NORSRAMTimingInitStructure.FSMC_AccessMode = FSMC_AccessMode_A;	/* FSMC ·AIEALE? */
...
void LCD_Initializtion(void)
{
	...
	LCD_Configuration();
	delay_ms(100);
	DeviceCode = LCD_ReadReg(0x0000);		/* ¶AC?CAID	*/
	/* ?»I¬CACy¶ZIC loEL»Z?»I¬ */
	if( DeviceCode == 0x9325 || DeviceCode == 0x9328 )
	{
		...
		LCD_WriteReg(0x0020,0x0000);  /* DDE?O·0 */
		LCD_WriteReg(0x0021,0x0000);  /* ADE?O·0 */
	}
	...
	{
	    LCD_Code = SSD1289;
	    LCD_WriteReg(0x0000,0x0001);    delay_ms(50);   /* ?nzSl§On */
	    ...
	    LCD_WriteReg(0x0001,0x2B3F);    delay_ms(50);   /* Cy¶ZEalozROC320*240 0x2B3F */
	    LCD_WriteReg(0x0002,0x0600);    delay_ms(50);
	    LCD_WriteReg(0x0010,0x0000);    delay_ms(50);
	    LCD_WriteReg(0x0011,0x6070);    delay_ms(50);   /* ¶?NlEylY?nE? 16I»E« saCA 0x6070 */
	    ...
	    LCD_WriteReg(0x000F,0x0000);    delay_ms(50);   /* E?AczSELµRO· */
	    ...
	    LCD_WriteReg(0x004f,0);        /* DDE?O·0 */
	    LCD_WriteReg(0x004e,0);        /* ADE?O·0 */
	}
	...
	else if( DeviceCode == 0x5408 )
	{
		...
		LCD_WriteReg(0x0003,0x1030);	  /* Entry ModeEcOA */
		...
		LCD_WriteReg(0x0050,0x0000);	  /* EcOA?U???°zUµAXOazSELAD */
		LCD_WriteReg(0x0051,0x00EF);	  /* EcOA?U???°zUµAXOa?aErAD */
		LCD_WriteReg(0x0052,0x0000);	  /* EcOA?U???°zUµAYOazSELDD */
		LCD_WriteReg(0x0053,0x013F);	  /* EcOA?U???°zUµAYOa?aErDD */

		LCD_WriteReg(0x0060,0x2700);	  /* Driver Output Control */
										  /* EcOACAA»µAµaEyNOL°E?AcµACdELDD */
...
* Description    : ?«CAA»EilalEO?¶?µANOE«L¬CcClCAL¬OnEila 0xffff
...
* Description    : RRRRRGGGGGGBBBBB ?AIS BBBBBGGGGGGRRRRR ?nE?
* Input          : - color: BRG NOE«Oµ
* Output         : None
* Return         : RGB NOE«Oµ
* Attention		 : AU?zsZEyµ?OA
*******************************************************************************/
...
{
    short dx,dy;      /* ¶?NlX YOaEDOoLOµA±aAzOµ */
    short temp;       /* Cdµa OOµa?oD?±C?D ?»»»EylYE±µAODLa±aAz */

    if( x0 > x1 )     /* XOaEDCdµa?oOUOOµa ?»»»EylY */
    {
	    temp = x1;
		x1 = x0;
		x0 = temp;
    }
    if( y0 > y1 )     /* YOaEDCdµa?oOUOOµa ?»»»EylY */
    {
		temp = y1;
		y1 = y0;
		y0 = temp;
	...
	dx = x1-x0;       /* XOa·?DnEDµAOoAz */
	dy = y1-y0;       /* YOa·?DnEDµAOoAz */

    if( dx == 0 )     /* XOaEDA»ODOoAz »aO±D? */
    {
        do
        {
            LCD_SetPoint(x0, y0, color);   /* OdµaDOEl Ac?aO±D? */
            y0++;
        }
        while( y1 >= y0 );
		return;
    }
    if( dy == 0 )     /* YOaEDA»ODOoAz »®C?O±D? */
    {
        do
        {
            LCD_SetPoint(x0, y0, color);   /* OdµaDOEl AcE®C?D? */
            x0++;
        }
        while( x1 >= x0 );
		return;
    ...
    if( dx > dy )                         /* zz?uXOa */
    {
	    temp = 2 * dy - dx;               /* LCEaDA?oµaµAI»OA */
        while( x0 != x1 )
        {
	        LCD_SetPoint(x0,y0,color);    /* » */
	        x0++;                         /* XOaEDLO1 */
	        if( temp > 0 )                /* LD¶DDADA?oµaµAI»OA */
	        {
	            y0++;                     /* ISONEDDrAUµaL¬L?L?x0+1,y0+1L© */
	            temp += 2 * dy - 2 * dx;
	 	    }
            else
            {
			    temp += 2 * dy;           /* LD¶DDADA?oµaµAI»OA */
			}
        }
        LCD_SetPoint(x0,y0,color);
    }
...
* Description    : ?«LcdCAEDCINaI»OADOElN»?o?O·u
* Input          : - Xpos: E®C??r±e
*                  - Ypos: ?aO±?r±e
*				   - ASCI: DOElµA?O·u
*				   - charColor: ?O·uNOE«
...
void PutChar( uint16_t Xpos, uint16_t Ypos, uint8_t ASCI, uint16_t charColor, uint16_t bkColor )
{
    ...
    GetASCIICode(buffer,ASCI);  /* C??OALEylY */
    for( i=0; i<16; i++ )
    {
        tmp_char = buffer[i];
        for( j=0; j<8; j++ )
        {
            ...
            {
                LCD_SetPoint( Xpos + j, Ypos + i, charColor );  /* ?O·uNOE« */
            }
            else
            {
                LCD_SetPoint( Xpos + j, Ypos + i, bkColor );  /* ±ll°NOE« */
            }
        }
    }
}
...
* Description    : ?«LcdCAEDCINaI»OADOElN»?oODIA?O
* Input          : - Xpos: E®C??r±e
*                  - Ypos: ?aO±?r±e
*				   - str: DOElµAODIA?O
*				   - Color: ?O·uNOE«
...
void PutChinese(uint16_t Xpos,uint16_t Ypos,uint8_t *str,uint16_t Color,uint16_t bkColor)
{
	...
	GetGBKCode(buffer,str);  /* C??OALEylY */
	...
	for ( i = 0; i < 16; i++ )
	{
		...
		for (j = 0; j < 16; j++ )
		{
	        ...
	        {
		        LCD_SetPoint(Xpos+j,Ypos+i,Color);  /* ?O·uNOE« */
	        }
	        else
	        {
	            LCD_SetPoint(Xpos+j,Ypos+i,bkColor);  /* ±ll°NOE« */
